...
public class checkforerror
{
   ...
   public static void main(String[] args) throws Exception //if still OOM subtract as you bring it in
   {
      ...
      System.out.println("correlationTime:" + (double)corrTime / 1000000000/(4)); //40 test instances are each correlated to their 39 other 39 test instances
  ...
  public static void report(ArrayList<Instancem> iterations)
   {
      ...
      //{
          int c= iterations.get(0).getS(); 
          int nn = nearestNeighbor(0,iterations);
          //valMatrix[c][nn]++;
  ...
  public static int mode(ArrayList<Integer> arr)
   ...
   {
      double p = 0.0;
         double tp = tp(valMatrix);
         double fp = fp(valMatrix,action);
         if(tp==0 && fp==0)
         {
            p=100;
         }
