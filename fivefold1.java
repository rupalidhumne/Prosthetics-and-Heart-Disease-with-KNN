...
public class fivefold
   {
      ...
      ArrayList<ArrayList<Double>> precision = new ArrayList<ArrayList<Double>>(5);
      ArrayList<ArrayList<Double>> recall = new ArrayList<ArrayList<Double>>(5);
      ...
      for(int x=0; x<5; x++)
         {
            ...
            for(int i=0; i<5; i++)
               {
                 pres(matrix);
                  //precision.set(i,pres(matrix));
               }    
            for(int r=0; r<5; r++)
               {
                  recall.add(r,recall(matrix)); //set the arraylist at index x in array list of precisions to be the arraylist returned by calculating the precisions from the confusion matrix                 
               }   
      ...
      for(int x=0; x<precision.size(); x++)
         {
      ...
      for(int x=0; x<sumpres.length; x++)
         {
            System.out.println("Precision for" +x+ ": " + sumpres[x]/sumpres.length);
         }
     double[] sumrecall = new double[5]; //sum of precision arrays
      for(int x=0; x<recall.size(); x++)
         {
      ...
      for(int x=0; x<sumrecall.length; x++)
         {
            System.out.println("Recall for" +x+ ": " + sumrecall[x]/sumrecall.length);
         }
      ...
      for(int x=0; x<temp.size(); x++)
         {
           ...
           double precision = Math.round(tp/(fp+tp));
           temp.add(precision); 
         }
       return temp;
   }        
