...
public class nthksim105
{
   ...
   static final int nCoords=640*480;
   //static final int nCoordsSize=55;
  ...
  */    
    	  ...
    	  for(int x=0; x<nIterations; x++) 
         {
            ...
            { 
              ...
              // int numCor=0;
               ...
               BufferedReader infile = new BufferedReader(new FileReader(filename)); // or use scanner and do i=0 to 100 infile.next() and then grab the 100th one
               //int numCol =0;
              // System.out.println("Reached!");
               for(int z=0; z<nRows; z++) //for every row
               {
                   ...
                   cols[i]=s.substring(pos+2, s.length());
                     //while(numCor<nCoords && coordArray.get(numCor).getRow()==z) //while there are coords dealing with this row
                     //{
      ...
      System.out.println("correlationTime:" + (double)corrTime / 1000000000/(54*55));
      System.out.println("patternTime:" + (double)patternTime / 1000000000/(54*55));
  ...
  public static void report(ArrayList<Instancem> iterations)
   ...
   }    public static int nearestNeighbor(int test, ArrayList<Instancem> train)
   {
	   ...
	   ArrayList<Double> corrs = new ArrayList<Double>();
	      ...
	      double time1 = System.nanoTime();
	      Double[][] avg1 =iterations.get(test).task2();
	      double time2=System.nanoTime();
